...
class MeshInfoFragment : BottomSheetDialogFragment() {
    ...
    override fun onCreateView(i:LayoutInflater,c:ViewGroup?,s:Bundle?):View{
        ...
        root.addView(LinearLayout(ctx).apply{orientation=LinearLayout.HORIZONTAL;gravity=android.view.Gravity.CENTER_VERTICAL;setPadding(20,14,16,6)
            ...
            addView(Button(ctx).apply{text="↻";textSize=12f;setTextColor(Color.parseColor("#00D4FF"));background=ctx.getDrawable(R.drawable.bg_card_dark);setPadding(16,0,16,0);layoutParams=LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT,40);setOnClickListener{loadStats()}})})
        ...
        val tv=TextView(ctx).apply{text="⏳ Loading…";textSize=11f;setTextColor(Color.parseColor("#9090B0"));typeface=android.graphics.Typeface.MONOSPACE
        ...
        tvContent=tv; root.addView(tv); loadStats(); return scroll
    }
    private fun loadStats(){(activity as? MainActivity)?.glView?.queueEvent{
        ...
        for(i in 0 until mc){val name=try{NativeLib.nativeGetMeshName(i)}catch(_:Exception){"Mesh #$i"}
            val s=try{NativeLib.nativeGetMeshStats(i)}catch(_:Exception){continue}
            if(s.size<9) continue; val wt=s[8]>0.5f
            sb.appendLine("── $name ──"); sb.appendLine("  Verts: ${s[5].toInt()}   Tris: ${s[6].toInt()}")
            sb.appendLine("  Box:   %.1f×%.1f×%.1f mm".format(s[2],s[3],s[4]))
            sb.appendLine("  Watertight: ${if(wt)"✓ Yes" else "✗ No"}"); if(i<mc-1)sb.appendLine()}
