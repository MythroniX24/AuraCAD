...
class MoveToolFragment : BottomSheetDialogFragment() {
    ...
    override fun onCreateView(i: LayoutInflater, c: ViewGroup?, s: Bundle?): View {
        ...
        root.addView(titleRow(ctx,"↕  Move","Reposition model along X/Y/Z axes"){
            posX=0f;posY=0f;posZ=0f;glRun{NativeLib.nativeSetTranslation(0f,0f,0f)}})
        root.addView(hdiv(ctx))
        root.addView(tip(ctx,"Drag sliders to reposition. 1 unit ≈ model diameter."))
            ...
            addView(SeekBar(ctx).apply{setMax(1000);progress=((init-min)/(max-min)*1000).toInt().coerceIn(0,1000)
                ...
                setOnSeekBarChangeListener(object:SeekBar.OnSeekBarChangeListener{
                    ...
                    override fun onProgressChanged(b:SeekBar,p:Int,fromUser:Boolean){
                        if(!fromUser) return; val v=min+p/1000f*(max-min); tvV.text="%.2f".format(v); cb(v)}})})}
    }
