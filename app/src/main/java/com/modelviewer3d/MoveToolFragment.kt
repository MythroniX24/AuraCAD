...
class MoveToolFragment:BottomSheetDialogFragment(){
    ...
    override fun onCreateView(i:LayoutInflater,c:ViewGroup?,s:Bundle?):View{
        ...
        for(ax in listOf(Ax("X  Left/Right",Color.parseColor("#FF9800"),{posX},{v->posX=v}),Ax("Y  Up/Down",Color.parseColor("#4CAF82"),{posY},{v->posY=v}),Ax("Z  Near/Far",Color.parseColor("#00D4FF"),{posZ},{v->posZ=v}))){
            ...
            root.addView(LinearLayout(ctx).apply{orientation=LinearLayout.VERTICAL;setPadding(20,10,20,4)
                addView(LinearLayout(ctx).apply{orientation=LinearLayout.HORIZONTAL;gravity=android.view.Gravity.CENTER_VERTICAL
                    addView(TextView(ctx).apply{text=ax.lbl;textSize=12f;setTextColor(Color.parseColor("#9090B0"));layoutParams=LinearLayout.LayoutParams(0,LinearLayout.LayoutParams.WRAP_CONTENT,1f)});addView(tv)})
