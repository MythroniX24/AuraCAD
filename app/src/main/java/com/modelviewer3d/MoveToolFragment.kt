...
class MoveToolFragment : BottomSheetDialogFragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        root.addView(LinearLayout(ctx).apply {
            ...
            addView(TextView(ctx).apply {
                text = "↕  Move  /  Position"; textSize = 17f
        ...
        root.addView(TextView(ctx).apply {
            text = "Drag sliders to reposition the model along each axis."
            textSize = 10f; setTextColor(Color.parseColor("#404060")); setPadding(20, 8, 20, 4)
        })

        root.addView(buildAxisSlider(ctx, "X  (Left/Right)", Color.parseColor("#FF9800"),
            -5f, 5f, posX) { v -> posX = v; glRun { NativeLib.nativeSetTranslation(posX, posY, posZ) } })
        root.addView(buildAxisSlider(ctx, "Y  (Up/Down)", Color.parseColor("#4CAF82"),
            -5f, 5f, posY) { v -> posY = v; glRun { NativeLib.nativeSetTranslation(posX, posY, posZ) } })
        root.addView(buildAxisSlider(ctx, "Z  (Near/Far)", Color.parseColor("#00D4FF"),
            -5f, 5f, posZ) { v -> posZ = v; glRun { NativeLib.nativeSetTranslation(posX, posY, posZ) } })
    ...
    private fun buildAxisSlider(
        ...
        val tvVal = TextView(ctx).apply {
            text = "%.2f".format(init); textSize = 11f
    ...
    companion object {
        const val TAG = "MoveTool"
        fun newInstance() = MoveToolFragment()
    }
}
