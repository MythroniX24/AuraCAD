...
class MoveToolFragment:BottomSheetDialogFragment(){
    ...
    override fun onCreateView(i:LayoutInflater,c:ViewGroup?,s:Bundle?):View{
        ...
        for((lbl,tint,get,set) in listOf(Triple("X  Left/Right",Color.parseColor("#FF9800"),{posX},{v:Float->posX=v}),Triple("Y  Up/Down",Color.parseColor("#4CAF82"),{posY},{v:Float->posY=v}),Triple("Z  Near/Far",Color.parseColor("#00D4FF"),{posZ},{v:Float->posZ=v}))){
            ...
            root.addView(LinearLayout(ctx).apply{orientation=LinearLayout.VERTICAL;setPadding(20,10,20,4)
                addView(LinearLayout(ctx).apply{orientation=LinearLayout.HORIZONTAL;gravity=android.view.Gravity.CENTER_VERTICAL
                    addView(TextView(ctx).apply{text=lbl.first;textSize=12f;setTextColor(Color.parseColor("#9090B0"));layoutParams=LinearLayout.LayoutParams(0,LinearLayout.LayoutParams.WRAP_CONTENT,1f)});addView(tv)})
                addView(SeekBar(ctx).apply{setMax(1000);progress=500
                    progressTintList=android.content.res.ColorStateList.valueOf(tint.second);thumbTintList=android.content.res.ColorStateList.valueOf(tint.second)
                    setOnSeekBarChangeListener(object:SeekBar.OnSeekBarChangeListener{override fun onStartTrackingTouch(b:SeekBar){};override fun onStopTrackingTouch(b:SeekBar){}
                        override fun onProgressChanged(b:SeekBar,p:Int,fu:Boolean){if(!fu)return;val v=-5f+(p/1000f)*10f;tv.text="%.2f".format(v);set.third(v);gl{NativeLib.nativeSetTranslation(posX,posY,posZ)}}})})})}
        return scroll
    }
