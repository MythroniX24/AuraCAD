    ...
    override fun onCreateView(i:LayoutInflater,c:ViewGroup?,s:Bundle?):View{
        ...
        root.addView(LinearLayout(ctx).apply{orientation=LinearLayout.HORIZONTAL;setPadding(14,8,14,0)
            for((lbl,ax,deg) in listOf(Triple("+90X","X",90f),Triple("−90X","X",-90f),Triple("180Y","Y",180f),Triple("180Z","Z",180f))){
                addView(Button(ctx).apply{text=lbl;textSize=9f;setTextColor(Color.parseColor("#A8B6C7"));background=ctx.getDrawable(R.drawable.bg_card_dark)
        ...
        for((lbl,tint) in listOf(Pair("X  Tilt",Color.parseColor("#FFB86B")),Pair("Y  Spin",Color.parseColor("#4CAF82")),Pair("Z  Roll",Color.parseColor("#62E6FF")))){
            val tv=TextView(ctx).apply{text="0°";textSize=11f;setTextColor(Color.parseColor("#62E6FF"))}
            root.addView(LinearLayout(ctx).apply{orientation=LinearLayout.VERTICAL;setPadding(20,10,20,4)
                addView(LinearLayout(ctx).apply{orientation=LinearLayout.HORIZONTAL;gravity=android.view.Gravity.CENTER_VERTICAL
                    addView(TextView(ctx).apply{text=lbl;textSize=12f;setTextColor(Color.parseColor("#A8B6C7"));layoutParams=LinearLayout.LayoutParams(0,LinearLayout.LayoutParams.WRAP_CONTENT,1f)});addView(tv)})
