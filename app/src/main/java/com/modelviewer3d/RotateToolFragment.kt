...
class RotateToolFragment : BottomSheetDialogFragment() {
    ...
    override fun onCreateView(i: LayoutInflater, c: ViewGroup?, s: Bundle?): View {
        ...
        val root=LinearLayout(ctx).apply{orientation=LinearLayout.VERTICAL
            setBackgroundResource(R.drawable.bg_bottom_sheet); setPadding(0,0,0,40)}
        scroll.addView(root)
        root.addView(handle(ctx))
        root.addView(titleRow(ctx,"🔄  Rotate","Spin model −180° to +180°"){
            rotX=0f;rotY=0f;rotZ=0f;glRun{NativeLib.nativeSetRotation(0f,0f,0f)}})
        root.addView(hdiv(ctx))
        root.addView(tip(ctx,"Drag sliders or use quick-preset buttons below."))
        ...
        root.addView(LinearLayout(ctx).apply{orientation=LinearLayout.HORIZONTAL;setPadding(14,8,14,0)
            for((lbl,ax,deg) in listOf(Triple("+90X","X",90f),Triple("−90X","X",-90f),Triple("180Y","Y",180f),Triple("180Z","Z",180f))){
                addView(Button(ctx).apply{text=lbl;textSize=9f;setTextColor(Color.parseColor("#9090B0"))
                    ...
                    setOnClickListener{when(ax){"X"->rotX=(rotX+deg).coerceIn(-180f,180f);"Y"->rotY=(rotY+deg).coerceIn(-180f,180f);"Z"->rotZ=(rotZ+deg).coerceIn(-180f,180f)}
    ...
    private fun axis(ctx:android.content.Context,lbl:String,tint:Int,min:Float,max:Float,init:Float,cb:(Float)->Unit):LinearLayout{
        val tvV=TextView(ctx).apply{text="%.0f°".format(init);textSize=11f;setTextColor(Color.parseColor("#00D4FF"))}
        return LinearLayout(ctx).apply{orientation=LinearLayout.VERTICAL;setPadding(20,10,20,4)
            addView(LinearLayout(ctx).apply{orientation=LinearLayout.HORIZONTAL;gravity=android.view.Gravity.CENTER_VERTICAL
                addView(TextView(ctx).apply{text=lbl;textSize=12f;setTextColor(Color.parseColor("#9090B0"))
            ...
            addView(SeekBar(ctx).apply{setMax(1000);progress=((init-min)/(max-min)*1000).toInt().coerceIn(0,1000)
                ...
                setOnSeekBarChangeListener(object:SeekBar.OnSeekBarChangeListener{
                    ...
                    override fun onProgressChanged(b:SeekBar,p:Int,fromUser:Boolean){
                        if(!fromUser) return; val v=min+p/1000f*(max-min); tvV.text="%.0f°".format(v); cb(v)}})})}
    }
    ...
    private fun titleRow(ctx:android.content.Context,title:String,sub:String,reset:()->Unit)=LinearLayout(ctx).apply{
        orientation=LinearLayout.HORIZONTAL;gravity=android.view.Gravity.CENTER_VERTICAL;setPadding(20,14,20,4)
        addView(LinearLayout(ctx).apply{orientation=LinearLayout.VERTICAL
            layoutParams=LinearLayout.LayoutParams(0,LinearLayout.LayoutParams.WRAP_CONTENT,1f)
            addView(TextView(ctx).apply{text=title;textSize=17f;setTypeface(null,android.graphics.Typeface.BOLD);setTextColor(Color.WHITE)})
            addView(TextView(ctx).apply{text=sub;textSize=10f;setTextColor(Color.parseColor("#505070"))})})
        addView(Button(ctx).apply{text="Reset";textSize=10f;setTextColor(Color.parseColor("#FF7043"))
    ...
    private fun hdiv(ctx:android.content.Context)=View(ctx).apply{setBackgroundColor(Color.parseColor("#1A1A28"))
        layoutParams=LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT,1)}
    private fun tip(ctx:android.content.Context,msg:String)=TextView(ctx).apply{text=msg;textSize=10f;setTextColor(Color.parseColor("#404060"));setPadding(20,8,20,4)}
